...
public class DataInitializer implements CommandLineRunner {
    ...
    private void seedCategories() {
        ...
        categories.put("Épicerie", "Produits d'épicerie du quotidien : farine, sucre, couscous, conserves.");
        categories.put("Boissons", "Toutes sortes de boissons rafraîchissantes, eaux et jus.");
        categories.put("Produits laitiers", "Lait, yaourts, fromages, beurre et produits dérivés.");
        categories.put("Thé Café & Infusions", "Thés verts, cafés, infusions et herbes aromatiques.");
        categories.put("Huiles & Olives", "Huiles de table, huiles d'olive et olives préparées.");
        categories.put("Produits de Nettoyage", "Produits d'entretien et de nettoyage pour la maison.");
        categories.put("Hygiène Corporelle", "Savons, shampoings, dentifrices et soins du corps.");
        categories.put("Papier & Fournitures", "Papier, mouchoirs, stylos et fournitures de bureau.");
        categories.put("Électronique", "Câbles, clés USB, imprimantes et équipements tech pour commerces.");

        int added = 0;
        for (Map.Entry<String, String> entry : categories.entrySet()) {
            try {
                if (categorieRepository.findByNom(entry.getKey()).isEmpty()) {
                    categorieRepository.save(Categorie.builder()
                            .nom(entry.getKey())
                            .description(entry.getValue())
                            .build());
                    System.out.println("  ➕ Catégorie créée: " + entry.getKey());
                    added++;
                }
            } catch (Exception e) {
                System.err.println("  ⚠️ Catégorie skip (" + entry.getKey() + "): " + e.getMessage());
            }
        }
        System.out.println("✅ Catégories: " + added + " créées, " + (categories.size() - added) + " existaient déjà.");
    }
    ...
    private void seedFournisseurs() {
        ...
        List<Map<String, String>> fournisseurs = List.of(
            ...
            Map.of("nom", "Fatima-Zahra Chraibi", "entreprise", "Laiterie du Nord",
                    "email", "fatimazahra.chraibi@gmail.com", "telephone", "+212 6 12 34 56 03",
        ...
        System.out.println("✅ Fournisseurs: " + added + " créés, " + (fournisseurs.size() - added) + " existaient déjà.");
    }
    ...
    private void seedClients() {
        ...
        List<Client> clientsToSeed = List.of(
            Client.builder().nom("Hamza Achhaiba").nomMagasin("Hanout Al Amal").email("hamza.achhaiba@gmail.com").telephone("+212 6 12 34 56 01").adresse("Avenue El Farah, Casablanca").role(Role.CLIENT).motDePasse(encodedPassword).build(),
            Client.builder().nom("Ahmed Oukhchine").nomMagasin("Mini Market Bennani").email("ahmed.oukhchine@gmail.com").telephone("+212 6 12 34 56 02").adresse("Rue Al Andalous, Rabat").role(Role.CLIENT).motDePasse(encodedPassword).build(),
            Client.builder().nom("Khadija Aitbella").nomMagasin("Épicerie Yassine").email("khadija.aitbella@gmail.com").telephone("+212 6 12 34 56 03").adresse("Avenue Hassan II, Fès").role(Role.CLIENT).motDePasse(encodedPassword).build(),
            Client.builder().nom("Hala Arbani").nomMagasin("Chraibi Express").email("hala.arbani@gmail.com").telephone("+212 6 12 34 56 04").adresse("Rue Ibn Sina, Marrakech").role(Role.CLIENT).motDePasse(encodedPassword).build(),
            Client.builder().nom("Bilal Othmani").nomMagasin("Othmani Shop").email("bilal.othmani.store@gmail.com").telephone("+212 6 12 34 56 05").adresse("Avenue Mohammed V, Tanger").role(Role.CLIENT).motDePasse(encodedPassword).build(),
            Client.builder().nom("Bader Tikert").nomMagasin("Tazi Al Baraka").email("bader.tikert@gmail.com").telephone("+212 6 12 34 56 06").adresse("Rue Al Amal, Meknès").role(Role.CLIENT).motDePasse(encodedPassword).build(),
            Client.builder().nom("Oussama El Fassi").nomMagasin("Mini Store El Fassi").email("oussama.elfassi.shop@gmail.com").telephone("+212 6 12 34 56 07").adresse("Avenue Al Massira, Agadir").role(Role.CLIENT).motDePasse(encodedPassword).build(),
            Client.builder().nom("Imad Berrada").nomMagasin("Berrada Market").email("imad.berrada.store@gmail.com").telephone("+212 6 12 34 56 08").adresse("Rue Al Qods, Oujda").role(Role.CLIENT).motDePasse(encodedPassword).build(),
            Client.builder().nom("Reda Ait Taleb").nomMagasin("Ait Taleb Services").email("reda.aittaleb.shop@gmail.com").telephone("+212 6 12 34 56 09").adresse("Avenue Moulay Ismail, Tétouan").role(Role.CLIENT).motDePasse(encodedPassword).build(),
            Client.builder().nom("Nabil Amrani").nomMagasin("Amrani Market").email("nabil.amrani.store@gmail.com").telephone("+212 6 12 34 56 10").adresse("Rue Al Wifaq, Kénitra").role(Role.CLIENT).motDePasse(encodedPassword).build(),
            Client.builder().nom("Hamza El Mansouri").nomMagasin("Hanout El Mansouri").email("hamza.almansouri.shop@gmail.com").telephone("+212 6 12 34 56 11").adresse("Avenue Al Jadida, El Jadida").role(Role.CLIENT).motDePasse(encodedPassword).build(),
            Client.builder().nom("Karim Boulahya").nomMagasin("Boulahya Mini Store").email("karim.boulahya.store@gmail.com").telephone("+212 6 12 34 56 12").adresse("Rue Al Bahr, Safi").role(Role.CLIENT).motDePasse(encodedPassword).build(),
            Client.builder().nom("Soufiane Azzouzi").nomMagasin("Azzouzi Market").email("soufiane.azzouzi.shop@gmail.com").telephone("+212 6 12 34 56 13").adresse("Avenue Al Qods, Nador").role(Role.CLIENT).motDePasse(encodedPassword).build(),
            Client.builder().nom("Rachid El Fassi").nomMagasin("Al Wifaq Store").email("rachid.elfassi.store@gmail.com").telephone("+212 6 12 34 56 14").adresse("Rue Al Farah, Mohammedia").role(Role.CLIENT).motDePasse(encodedPassword).build(),
            Client.builder().nom("Omar Alaoui").nomMagasin("Alaoui Commerce").email("omar.alaoui.shop@gmail.com").telephone("+212 6 12 34 56 15").adresse("Avenue Atlas, Béni Mellal").role(Role.CLIENT).motDePasse(encodedPassword).build()
        );
        ...
        System.out.println("✅ Clients: " + added + " créés, " + (clientsToSeed.size() - added) + " existaient déjà.");
    }
    ...
    private void seedProducts() {
        ...
        // Each entry: { productName, supplierEmail, categoryName, imageUrl, price, description }
        List<String[]> products = List.of(
            ...
            new String[]{"Lait UHT Centrale", "fatimazahra.chraibi@gmail.com", "Produits laitiers",
            ...
            new String[]{"Yaourt Danone (Assil)", "fatimazahra.chraibi@gmail.com", "Produits laitiers",
            ...
            new String[]{"Le beurre Président", "fatimazahra.chraibi@gmail.com", "Produits laitiers",
            ...
            new String[]{"Fromage La Vache qui Rit", "fatimazahra.chraibi@gmail.com", "Produits laitiers",
            ...
            new String[]{"Huile d'olive Oued Souss", "mehdi.elidrissi@gmail.com", "Huiles & Olives",
            ...
            new String[]{"Olives rouges pimentées", "mehdi.elidrissi@gmail.com", "Huiles & Olives",
            ...
            new String[]{"Nettoyant multi-usages Sanicroix Maxis'", "khadija.tazi@gmail.com", "Produits de Nettoyage",
            ...
            new String[]{"Papier toilette Dalia", "amine.bennis@gmail.com", "Papier & Fournitures",
                "https://m.media-amazon.com/images/I/81vwMu7pK6S.jpg",
                "28.00", "Papier toilette Dalia double épaisseur, pack 12 rouleaux."},
            new String[]{"Mouchoirs en papier Dalaa", "amine.bennis@gmail.com", "Papier & Fournitures",
                "https://liya.ma/wp-content/uploads/2022/02/1-41-1.jpg",
                "8.00", "Mouchoirs en papier Dalaa, boîte de 200 feuilles."},
            new String[]{"Carnets de crédit (Karni)", "amine.bennis@gmail.com", "Papier & Fournitures",
            ...
            new String[]{"Rouleaux de caisse thermique", "hamza.elfassi@gmail.com", "Électronique",
            ...
            new String[]{"Lecteurs de codes-barres", "hamza.elfassi@gmail.com", "Électronique",
                ...
                "350.00", "Lecteur de codes-barres USB filaire, compatible tous systèmes."},
            ...
            new String[]{"Onduleurs (UPS)", "hamza.elfassi@gmail.com", "Électronique",
                ...
                "850.00", "Onduleur UPS 1000VA, protection contre les coupures de courant."}
        );
        ...
        for (String[] p : products) {
            ...
            try {
                ...
                if (user == null || !(user instanceof Fournisseur)) {
                    System.err.println("  ⚠️ Fournisseur non trouvé: " + supplierEmail + " — produit ignoré: " + productName);
                    continue;
                }
                Fournisseur fournisseur = (Fournisseur) user;

                // Check duplicate
                if (produitRepository.existsByNomAndFournisseurId(productName, fournisseur.getId())) {
                ...
                if (categorie == null) {
                    System.err.println("  ⚠️ Catégorie non trouvée: " + categoryName + " — produit ignoré: " + productName);
                    continue;
                }
                ...
                try {
                    Path localPath = downloadImage(imageUrl, fileName);
                    if (localPath != null) {
                        localImageUrl = BASE_URL + "/uploads/produits/" + fileName;
                        imageSuccess++;
                    }
                } catch (Exception imgEx) {
                    System.err.println("  ⚠️ Image download failed for " + productName + ": " + imgEx.getMessage());
                    imageFailed++;
                }

                // If image download failed, use the remote URL as fallback
                ...
                System.out.println("  ➕ Produit créé: " + productName + " → " + fournisseur.getNomEntreprise());
                added++;
        ...
        System.out.println("✅ Produits: " + added + " créés, " + (products.size() - added) + " existaient déjà.");
        System.out.println("📸 Images: " + imageSuccess + " téléchargées, " + imageFailed + " en fallback URL distante.");
    }
     ...
     */
    private Path downloadImage(String imageUrl, String targetFileName) {
        try {
            ...
            // Handle redirects manually for HTTPS→HTTP and vice versa
            ...
            if (responseCode == 200) {
                ...
                System.err.println("    HTTP " + responseCode + " for: " + imageUrl);
                return null;
            }
        } catch (Exception e) {
    ...
    /**
     * Sanitizes a product name into a safe filesystem-friendly filename.
     */
    ...
    /**
     * Guesses the file extension from the image URL.
     */
    private String guessExtension(String url) {
        ...
        // Default to jpg for most product images
        return ".jpg";
    }
}
